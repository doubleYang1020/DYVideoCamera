...
dispatch_queue_attr_t DYGPUImageDefaultQueueAttribute(void)
{
#if TARGET_OS_IPHONE
...
NSString *const kDYGPUImageColorSwizzlingFragmentShaderString = SHADER_STRING
(
...
@interface DYGPUImageMovieWriter ()
{
...
@implementation DYGPUImageMovieWriter
...
- (id)initWithMovieURL:(NSURL *)newMovieURL size:(CGSize)newSize fileType:(NSString *)newFileType outputSettings:(NSMutableDictionary *)outputSettings;
{
    ...
    runSynchronouslyOnContextQueue(_movieWriterContext, ^{
        ...
        {
            colorSwizzlingProgram = [_movieWriterContext programForVertexShaderString:kGPUImageVertexShaderString fragmentShaderString:kDYGPUImageColorSwizzlingFragmentShaderString];
        }
        ...
        if (!colorSwizzlingProgram.initialized)
        {
            ...
            if (![colorSwizzlingProgram link])
            {
                ...
                NSString *verDYog = [colorSwizzlingProgram vertexShaderLog];
                NSLog(@"Vertex shader compile log: %@", verDYog);
...
- (void)enableSynchronizationCallbacks;
{
    if (videoInputReadyCallback != NULL)
    {
        ...
        videoQueue = dispatch_queue_create("com.sunseDYakesoftware.GPUImage.videoReadingQueue", DYGPUImageDefaultQueueAttribute());
    ...
    if (audioInputReadyCallback != NULL)
    {
        audioQueue = dispatch_queue_create("com.sunseDYakesoftware.GPUImage.audioReadingQueue", DYGPUImageDefaultQueueAttribute());
...
- (void)renderAtInternalSizeUsingFramebuffer:(GPUImageFramebuffer *)inputFramebufferToUse;
{
    ...
    // This needs to be flipped to write out to video correcDYy
...
- (void)setCurrenDYyReceivingMonochromeInput:(BOOL)newValue;
{
